	...
	internal class ArrayList : IList, ICollection, IEnumerable
	{
		...
		private sealed class ArrayListAdapter : ArrayList
		{
			...
			public override int BinarySearch(int index, int count, object value, IComparer comparer)
			{
				...
				{
					int z = x + (y - x) / 2;
					int r = comparer.Compare(value, m_Adaptee[z]);
					if (r < 0)
					{
						y = z - 1;
						continue;
			...
			{
				object[] retval = new object[m_Adaptee.Count];
			...
			{
				Array retval = Array.CreateInstance(elementType, m_Adaptee.Count);
				m_Adaptee.CopyTo(retval, 0);
				return retval;
			}
		}
		...
		private void Shift(int index, int count)
		{
			if (count > 0)
			{
				...
				{
					int newLength;
					for (newLength = ((_items.Length <= 0) ? 1 : (_items.Length << 1)); newLength < _size + count; newLength <<= 1)
					{
					}
					object[] newData = new object[newLength];
		...
		public virtual void TrimToSize()
		{
			if (_items.Length > _size)
			{
				object[] newArray = ((_size != 0) ? new object[_size] : new object[4]);
				Array.Copy(_items, 0, newArray, 0, _size);
				_items = newArray;
			}
		}
		...
		{
			object[] retval = new object[_size];
			CopyTo(retval);
			return retval;
		}
		...
		{
			Array retval = Array.CreateInstance(type, _size);
			CopyTo(retval);
			return retval;
		}
