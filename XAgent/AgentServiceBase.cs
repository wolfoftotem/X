...
{
    #region 构造
    static AgentServiceBase() => _Instance ??= new TService();
    #endregion
                ...
                switch (key.KeyChar - '0')
                {
                    ...
                    case 4:
                        ...
                        try
                        {
                            ...
                            {
                                for (var i = 0; i < Instance.ThreadCount; i++)
                                {
                                    service.Work(i);
                                }
                            }
    ...
    /// <summary>线程组</summary>
    private Thread[] Threads { get => _Threads ??= new Thread[ThreadCount]; set => _Threads = value; }

    private Dictionary<String, IServer> _AttachServers;
    /// <summary>附加服务</summary>
    public Dictionary<String, IServer> AttachServers => _AttachServers ??= new Dictionary<String, IServer>();
    ...
    //                if (type != null)
    //                {
    //                    if (TypeX.CreateInstance(type) is IServer service) AttachServers[item.Key] = service;
    //                }
    //            }
    //        }
    ...
    public virtual Boolean Work(Int32 index) => false;
